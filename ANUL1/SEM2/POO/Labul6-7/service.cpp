...
void service::addFilm(string titlu, string gen, int anul_aparitiei, string actor_principal) {
	...
	val.validate(f);
	rep.adauga(f);
	undoList.push_back(new UndoAdauga(f, rep));
}
...
void service::deleteFilm(string titlu) {
	...
	for (const auto& film : filme)
		if (film.getTitlu() == titlu)
			f = film;
	undoList.push_back(new UndoSterge(f, rep));
	rep.sterge(f);
}
...
void service::modifyFilm(string titlu1, string titlu2) {
	...
	for (const auto& film : filme)
		if (film.getTitlu() == titlu1)
			f1 = film;
	undoList.push_back(new UndoModifica(f1,f2,rep));
	rep.modifica(f1,f2);
}
...
int service::countFilm(string gen)
{
	vector<Film> all{ rep.getAll() };
	return (int)count_if(all.begin(), all.end(), [gen](const Film& film) {
		return gen == film.getGen(); });
}

void service::adaugaInCos(string titlu)
{
	cos.adauga(titlu);
}
...
void service::exportCos(const char* file)
{
	cos.exportC(file);
}
...
service::~service()
{
	for (ActiuneUndo* actiune : undoList)
		delete actiune;
}
