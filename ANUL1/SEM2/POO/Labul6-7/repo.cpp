...
void repo::adauga(const Film& f)
{
	if (find_if(all.begin(), all.end(),
		[&f](const Film& film) noexcept {return f.getTitlu() == film.getTitlu(); }) != all.end())
...
void repo::sterge(const Film& f) {
	...
	auto it = find_if(all.begin(), all.end(),
		[f](const Film& film) noexcept {return f.getTitlu() == film.getTitlu(); });
	if (it != all.end())
		all.erase(it);
...
bool repo::cauta(const Film& f) {
	auto it = find_if(all.begin(), all.end(), [f](const Film& film) {return f.getTitlu() == film.getTitlu(); });
	if (it != all.end()) 
		return true; 
	else 
		return false; }
...
void repo::modifica(Film& f1, const Film& f2) {
	...
	auto it = find_if(all.begin(), all.end(), [f1](const Film film) { return f1.getTitlu() == film.getTitlu(); });
	if (it != all.end())
		(*it).setTitlu(f2.getTitlu());
	else
		throw repoException("Filmul nu exista!\n");
